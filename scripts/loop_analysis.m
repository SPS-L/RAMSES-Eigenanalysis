...
global S Jdyn_sp dif_states P raw_vars dom_V fig dom_eigenvals DF analysis

exit=false;
while ~exit
    exit=false;
    if ~strcmp(analysis,'QZ')
        fprintf('\nWARNING: You use the ARP method. The Participation Factor and Mode Shape are not available.\n');
    ...
    if strcmp(reply, 'MS')
        if ~strcmp(analysis,'QZ')
            fprintf('This is not available in the ARP method, only in QZ...\n');
            continue
        end  
        fig=fig+1;
        cc=hsv(12);
